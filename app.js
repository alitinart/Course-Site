...
app.delete("/course/:id", (req, res) => {
  Course.findOneAndDelete({ _id: req.params.id })
    .then((removedCourseDoc) => res.send(removedCourseDoc))
    .catch((err) => {
      res.sendStatus(400).send(err);
    });
});
...
app.patch("/course/:id", (req, res) => {
  Course.findByIdAndUpdate({ _id: req.params.id }, { $set: req.body })
    .then((course) => res.send(course))
    .catch((err) => {
      res.sendStatus(400).send(err);
    });
...
app.delete("/course/:id/:videoTitle", (req, res) => {
  Video.findOneAndDelete({ title: req.params.videoTitle })
    .then((removedCourseDoc) => res.send(removedCourseDoc))
    .catch((err) => {
      res.sendStatus(400).send(err);
    });
...
app.post("/token", (req, res) => {
  ...
  RefreshTokenModel.find({ token: refreshToken })
    .then((tokens) => {
      if (tokens.length <= 0) {
        res.sendStatus(403);
      }
